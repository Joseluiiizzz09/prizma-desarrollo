...
function esVentaInstalada(venta) {
  ...
  return estado === 'INSTALADO' || estado === 'INSTALADO NO VALIDADO' || estado === 'REASIGNACION' || estado === 'EJECUTADA'
}
                    ...
                    <div key={f.id || f.url} style={{border:'1px solid #e5e7eb',borderRadius:'12px',overflow:'hidden',background:'#f9fafb'}}>
                      ...
                      {f.id && (
                        <button onClick={() => eliminarFoto(f.id, modalFotos.ventaId)}
                          style={{width:'100%',padding:'4px',border:'none',background:'#fff7ed',color:'#ea580c',fontSize:'10px',fontWeight:700,cursor:'pointer'}}>
                          Eliminar
                        </button>
                      )}
                    </div>
