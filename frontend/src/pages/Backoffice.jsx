...
const TIPIF_VEND_STYLES = {
  ...
  'NO CONTESTA':['#ffffd6','#737300'],'BUZON DE VOZ':['#fff7ed','#9a3412'],'CORTA LLAMADA':['#e7f5fb','#236b8d'],
  'EN EJECUCION':['#eeeeee','#3f3f3f'],'SIN COBERTURA':['#ffedd5','#9a3412'],'NO CALIFICA':['#fff0e8','#8a4529'],
...
const TIPIF_VEND_FUERTE = {
  ...
  'NO CALIFICA':['#ffedd5','#9a3412','#fdba74'], 'SIN COBERTURA':['#ffedd5','#c2410c','#fdba74'], 'EDIFICIO NO LIBERADO':['#ffedd5','#9a3412','#fdba74'],
  'NO TOCAR':['#ffedd5','#980000','#fdba74'], 'FRAUDE':['#ffedd5','#9a3412','#fdba74'], 'NO ROTAR':['#ffedd5','#980000','#fdba74'], 'SH NO ROTAR':['#ffedd5','#980000','#fdba74'], 'SH NO TOCAR':['#ffedd5','#980000','#fdba74'],
}
          ...
          <div style={{ maxHeight:158, overflowY:'auto', scrollbarGutter:'stable' }}>
            ...
            {lista.map(a=>(
              <div key={a.id} onMouseDown={e=>e.preventDefault()} onClick={()=>elegir(a.nombre)}
                style={{ padding:'6px 8px', cursor:'pointer', fontSize:12, borderRadius:6, fontWeight: a.nombre===value?700:400, background: a.nombre===value?'#fff7ed':'transparent' }}>
                {a.nombre}
              </div>
            ))}
      ...
      <div className={`bo-layout${sidebarAbierto ? '' : ' sidebar-cerrado'}`}>
        ...
        <main className="bo-main">
                                ...
                                : rotVistaAsignacion.map(l => {
                                  ...
                                  return (
                                    <tr key={l.id} className={(prohibido||(!apto&&rotAsesor))?'row-noapto':''}>
                                      ...
                                      <td className="hora-color" style={{color:l.fechaAsignacion===fechaHoy()?'#f97316':'#111827'}}><span style={{display:'block',whiteSpace:'nowrap'}}>{formatFecha(l.fechaAsignacion)}</span><span style={{display:'block',whiteSpace:'nowrap'}}>{l.ultimaAsig.toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'})}</span></td>
            ...
            <div className="base-tabla-wrap">
                         ...
                         return [
                          <tr key={r.id} id={`fila-${r.id}`}>
                            ...
                            <td style={{textAlign:'center'}}>
                              {horaAsignacion
                                ?<span className="hora-cell" style={{color:esAsignacionHoy?'#f97316':'#111827'}}><span style={{display:'block',whiteSpace:'nowrap'}}>{formatFecha(fechaAsignacion)}</span><span style={{display:'block',whiteSpace:'nowrap'}}>{horaAsignacion}</span></span>
                              ...
                              <div className="historial-inner">
                                ...
                                {(() => {
                                  ...
                                  return (<>
                                              ...
                                              <div className="hist-content">
                                                <div className="hist-title" style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
                                                  ...
                                                  <button type="button" title="Eliminar esta asignación" onClick={()=>eliminarAsignacion(r.id, h)}
                                                    style={{fontSize:10,padding:'2px 8px',border:'1px solid #f97316',color:'#f97316',background:'#fff',borderRadius:5,cursor:'pointer',fontWeight:600,whiteSpace:'nowrap'}}>
                                                    Quitar
                                                  </button>
                                                </div>
                                    ...
                                    {retiros.map((h,ri)=>(
                                      <div key={`retiro-${ri}`} style={{fontSize:11,color:'#9a3412',marginTop:8,padding:'6px 8px',background:'#fff7ed',borderRadius:6,borderLeft:'3px solid #f97316'}}>
                                ...
                                <div style={{marginTop:10, textAlign:'right'}}>
                                  <button type="button"
                                    ...
                                    style={{fontSize:11, padding:'4px 12px', border:'1px solid #f97316', color:'#f97316', background:'#fff', borderRadius:6, cursor:'pointer', fontWeight:600}}>
                                    Eliminar número
                                  </button>
                                </div>
                              </div>
              ...
              <div style={{borderTop:'1px solid #e5e7eb',padding:'12px 14px',background:'#f8fafc'}}>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  {[
                    ['sin_cobertura','SIN COBERTURA',gruposProtegidos.sin_cobertura.length,'#ea580c'],
          ...
          <section className={`bo-seccion${seccion==='carga-masiva'?'':' hidden'}`}>
                    ...
                    <>
                      ...
                      <div style={{marginTop:10,maxHeight:200,overflowY:'auto',border:'1px solid #e5e7eb',borderRadius:8,background:'#fff'}}>
                        <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                          ...
                          <tbody>
                            {masivaFilas.map((f,i)=>(
                              <tr key={i} style={{borderBottom:'1px solid #f3f4f6',background:f.dup?'#fff7ed':''}}>
                                ...
                                <td style={{padding:'5px 10px'}}>
                                  {f.dup
                                    ? <><span style={{background:'#ffedd5',color:'#9a3412',fontSize:9,fontWeight:700,padding:'2px 7px',borderRadius:99}}>DUPLICADO</span> <span style={{fontSize:9,color:'#c2410c'}}>{f.motivo}</span></>
                ...
                <div>
                  ...
                  {legacyUsarFecha === 'si' ? (
                    ...
                    <div className="bo-input-group" style={{margin:'0 0 14px'}}>
                      ...
                      />
                      {legacyFecha > fechaHoy() && <div style={{fontSize:11,color:'#ea580c',marginTop:4}}>No se permiten fechas futuras.</div>}
                    </div>
                  )}
                        ...
                        <div>
                          ...
                          {legacyUsarFecha==='si' && fechasDetectadas.length > 1 ? (
                            <div style={{fontSize:11,color:'#92400e'}}>
                              ...
                              {errorRows.length > 0 && <span style={{marginLeft:8,color:'#ea580c',fontWeight:700}}> · {errorRows.length} con error de fecha</span>}
                            </div>
                          ) : (
                            <div style={{fontSize:11,color:'#92400e'}}>
                              Fecha destino: <strong>{legacyUsarFecha==='si' ? formatFecha(legacyRows.find(r=>!r._fechaError)?.fecha||'') : formatFecha(legacyFecha)}</strong>
                              <span style={{margin:'0 8px',color:'#fed7aa'}}>·</span>
                              {validRows.length} válidos{errorRows.length>0&&<span style={{color:'#ea580c',fontWeight:700}}> · {errorRows.length} con error</span>}
                            </div>
                          )}
                        </div>
                        ...
                        <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,whiteSpace:'nowrap'}}>
                          ...
                          <tbody>
                            {legacyRows.slice(0,80).map((r,i)=>(
                              <tr key={i} style={{borderBottom:'1px solid #f3f4f6',background:r._fechaError?'#fff7ed':''}}>
                                <td style={{padding:'4px 10px',color:'#9ca3af'}}>{i+1}</td>
                                {legacyUsarFecha==='si' && (
                                  <td style={{padding:'4px 10px',fontWeight:700,color:r._fechaError?'#ea580c':'#1d4ed8',minWidth:85}}>
                                ...
                                {legacyUsarFecha==='si' && (
                                  <td style={{padding:'4px 10px'}}>
                                    {r._fechaError
                                      ? <span style={{background:'#ffedd5',color:'#9a3412',fontSize:9,fontWeight:700,padding:'2px 6px',borderRadius:99}}>ERROR</span>
            ...
            {legacyError && (
              <div style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:10,padding:14,marginTop:4,display:'flex',alignItems:'flex-start',gap:10}}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ea580c" strokeWidth="2.2" style={{flexShrink:0,marginTop:1}} aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:'#ea580c',marginBottom:2}}>Error al importar</div>
                  <div style={{fontSize:11,color:'#7c2d12'}}>{legacyError}</div>
                </div>
              </div>
            )}
              ...
              <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:12,padding:18,marginTop:4}}>
                ...
                {erroresDetalle && erroresDetalle.length > 0 && (
                  <div style={{borderTop:'1px solid #bbf7d0',paddingTop:10,marginBottom:10}}>
                    <div style={{fontSize:11,fontWeight:700,color:'#ea580c',marginBottom:6}}>Registros con error ({erroresDetalle.length}):</div>
                    <div style={{maxHeight:100,overflowY:'auto',fontSize:10,color:'#7c2d12'}}>
        ...
        <div className="modal-overlay open" onClick={e=>{ if(e.target===e.currentTarget) setModalRotar(p=>({...p,open:false})) }}>
          <div className="modal-box">
            ...
            {(() => {
              ...
              return (
                <div style={{border:`1px solid ${rotModalAsesor?'#e5e7eb':'#f97316'}`, borderRadius:10, padding:8, marginBottom:10}}>
                  ...
                  <div style={{maxHeight:170, overflowY:'auto', marginTop:6}}>
                    {filtrados.map(a=>(
                      <div key={a.id} onClick={()=>{ setRotModalAsesor(a.nombre); setRotModalError('') }}
                        style={{padding:'7px 9px', cursor:'pointer', fontSize:13, borderRadius:7, fontWeight:a.nombre===rotModalAsesor?700:400, background:a.nombre===rotModalAsesor?'#fff7ed':'transparent', color:a.nombre===rotModalAsesor?'#c2410c':'#111827'}}>
            ...
            {rotModalError && <div role="alert" style={{marginTop:8,padding:'9px 11px',border:'1px solid #fed7aa',borderRadius:8,background:'#fff7ed',color:'#c2410c',fontSize:12,fontWeight:650}}>{rotModalError}</div>}
            <div className="modal-btns">
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                ...
                <tbody>
                  {blCargando
                    ...
                    : !blLeads
                      ? <tr><td colSpan={10} style={{textAlign:'center',padding:40,color:'#f97316'}}>Error de conexión.</td></tr>
