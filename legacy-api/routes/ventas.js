...
const ESTADOS_VALIDOS_PATCH = [
  ...
  'EN_PROGRESO','PROGRAMADA','REPROGRAMADO','SIN_INGRESO','DESAPROBADO',
];
...
router.get('/', auth(ROLES_VENTAS), async (req, res) => {
  try {
    ...
    if (seguimiento_campo === '1') {
      sql += ` AND UPPER(TRIM(v.estado)) IN ('EN_EJECUCION','INSTALADO','CAIDA','RECHAZO_CAMPO','TECNICO_CASA','LEVANTAR_SOT','TECNICOS_CAMINO','INSTALADO_NO_VALIDADO','REASIGNACION','DERIVADO_PLANTA_EXTERNA','SERVICIO_ACTIVO','RECHAZO_MESA','EN_PROGRESO','PROGRAMADA','REPROGRAMADO','SIN_INGRESO','DESAPROBADO')`;
    }
